...
The [Tuple](https://en.wikipedia.org/wiki/Tuple) is nothing new, natively implemented in some languages like Python, Lisp, Scala and the list continues.
...
A running version of these example can be found in the _example/darpule.dart_. Refer to that, the test suite and API Documentation for a complete understanding.
...
The immutability comes view Dart's [UnmodifiableListView](https://api.dartlang.org/134830/dart-collection/UnmodifiableListView-class.html), which has a small reasonably footprint. We can place anything we like into our Tuple.  While still having access to Darts functional goodness.
...
_Gives us. The quintuple is melted away in this new tuple_. Of course ```signalPartB`` has not mutated in any way.
...
Tuple extends [UnmodifiableListView](https://api.dartlang.org/134830/dart-collection/UnmodifiableListView-class.html), which comes from LiseBase and mixes in ListMixin. Hence the Tuple is just a gloried ```List``` and you use it accordingly. 
...
The Matcher is aware of [Optional](https://www.dartdocs.org/documentation/quiver_optional/1.0.0-dev.1/quiver.optional/Optional-class.html) from the Quiver Library. The matcher interprets the Optional as meaning that the element does have to present on in the subject. When present the element needs to match the value or type specified in the Optional.
...
- An embedded criteria Tuple within a Criteria, not bullet proof yet... _This is top property._ 
...
- Optional is expected to change in quiver after the arrival of [NNBD (Non-null Types & Non-null By Default)](https://github.com/chalin/DEP-non-null), some [migration strategies](https://github.com/chalin/DEP-non-null/blob/master/doc/dep-non-null-AUTOGENERATED-DO-NOT-EDIT.md#part-migration) have been thought about.  
